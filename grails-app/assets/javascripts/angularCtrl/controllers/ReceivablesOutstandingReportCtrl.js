...
angular.module("app.ReceivablesOutstandingReportCtrl", [])
    .controller("ReceivablesOutstandingReportCtrl", ["$scope", "$rootScope", "httpInfo", "httpService", "$routeParams", "$location", "logger", "AccountLedgerService", function ($scope, $rootScope, httpInfo, httpService, $routeParams, $location, logger, ALService) {
        init();

        function init() {
            $scope.groupObject = {};
            $scope.groupObject.property = "Sundry Debtors";
            $scope.groupObject.reportName = "Bills Receivable";
