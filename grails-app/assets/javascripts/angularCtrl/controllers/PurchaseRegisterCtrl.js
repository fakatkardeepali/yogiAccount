 ...
 */
angular.module("app.PurchaseRegisterCtrl", [])
    .controller("PurchaseRegisterCtrl", ["$scope", "$http", "$location", "httpInfo", "httpService", "$routeParams", "$rootScope", "logger", function ($scope, $http, $location, httpInfo, httpService, $routeParams, $rootScope, logger) {

        init();

        function init() {
            $scope.groupObject = {};
            $scope.groupObject.property = "Purchase";
            $scope.templatePath = "saleRegisterReportTemplate.html"
            $scope.isGroupVoucher = false;
            var params = {
                method: httpInfo.get,
                url: "../voucher/findVoucherType",
                data: httpService.toParams({property: "Purchase"}),
