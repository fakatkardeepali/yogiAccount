...
angular.module("app.StatisticsReportCtrl", [])
    .controller("StatisticsReportCtrl", ["$scope", "$rootScope", "httpInfo", "httpService", "$routeParams", "$location", "logger", "AccountLedgerService", function ($scope, $rootScope, httpInfo, httpService, $routeParams, $location, logger, ALService) {
        init();

        function init() {
            $scope.groupObject = {};
            $scope.templatePath = "statisticsReport.html"

            var params = {
                method: httpInfo.get,
                url: "../voucher/generateStatisticsReport",
