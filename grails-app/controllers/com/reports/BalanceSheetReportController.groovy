...
@ParentScreen(name = "Outstanding", fullName = "Balance Sheet", sortList = 1, link = "/reports/balanceSheet")
class BalanceSheetReportController {
    ...
    def generateBalanceSheetReport() {
        ...
        if (arr) {
            data = masterReportService.findAllBalanceSheetReport(arr, session["company"], paramsData.fromDate, paramsData.toDate);
            if (data) {
                reportDetails.push(data)
            }
            params._file = "../reports/balanceSheetReport"
        }
        params.SUBREPORT_DIR = "${servletContext.getRealPath('/reports')}/"
        chain(controller: 'balanceSheetReport', action: 'generateReport', model: [data: reportDetails], params: params)
    }
